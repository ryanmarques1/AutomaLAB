...
def desenhar_automato(estado_inicial, estados_finais, delta):
    ...
    automato.node(estado_inicial, shape='circle', fontsize='18', fontcolor='black') #edefinindo o nó inicial estado inicial receber circulo
    for estado_final in estados_finais: 
        automato.node(estado_final, shape='doublecircle', fontsize='18', fontcolor='green')#colocando circulo duplo em todos os estados finais.
...
def armazena_arquivo(pasta, delta):
    arqAUTOMATO = open(pasta + ('automatoCriado.txt'), 'w')
